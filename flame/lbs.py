...
def lbs(betas, pose, v_template, shapedirs, posedirs, J_regressor, parents,
    ...
    # ---- 自己加的 ----
    v_template = v_template.to('cuda')
    shapedirs = shapedirs.to('cuda')
    betas = betas.to('cuda')
    # -----------------
    ...
    # NxJx3 array
    # ---------
    J_regressor = J_regressor.to('cuda')
    v_shaped = v_shaped.to('cuda')
    # ---------
    ...
    # --------
    pose_feature = pose_feature.to('cuda')
    posedirs = posedirs.to('cuda')
    # --------
    ...
    # -----------
    v_posed = v_posed.to('cuda')
    homogen_coord = homogen_coord.to('cuda')
    # -----------
...
def forward_skinning_pts(pnts_c, transformations, lbs_weights, dtype=torch.float32, mask=None):
    ...
    # transformations = torch.randn(1, 6, 4, 4).float()
    transformations = transformations.to('cuda')
    W = W.to('cuda')
...
def blend_shapes(betas, shape_disps):
    ...
    if len(shape_disps.shape) == 3:
        # --------
        betas = betas.to('cuda')
        shape_disps = shape_disps.to('cuda')
        # --------
...
def batch_rigid_transform(rot_mats, joints, parents, dtype=torch.float32):
    ...
    # ----------
    rot_mats = rot_mats.to('cuda')
    rel_joints = rel_joints.to('cuda')
    # ----------
