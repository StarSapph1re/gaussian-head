...
class Scene:
    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, is_debug, novel_view, only_head, load_iteration=None, shuffle=True,
                 resolution_scales=[1.0], reenact_path=None):
        """b
        ...
        if os.path.exists(os.path.join(args.source_path, "transforms.json")):
            scene_info = sceneLoadTypeCallbacks["nerfblendshape"](args.source_path, args.eval, is_debug, novel_view, only_head, reenact_path)
        else:
            assert False, "Could not recognize scene type!"

        self.num_points = scene_info.point_cloud.points
        self.mean_exp = scene_info.mean_exp
        self.var_exp = scene_info.var_exp
        self.shape_params = scene_info.shape_params
