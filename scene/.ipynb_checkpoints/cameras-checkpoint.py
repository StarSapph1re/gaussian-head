...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask, image_name, uid,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device="cuda", exp=None, pose=None, depth=None):
        ...
        self.pose = torch.from_numpy(pose).type(torch.FloatTensor).to(self.data_device)
    ...
    def load2device(self, data_device='cuda'):
        ...
        self.pose = self.pose.to(data_device)
